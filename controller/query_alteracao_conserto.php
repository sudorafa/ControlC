...
if($situacao1 == "filial") {
	if($situacao1 == $situacao_banco1)
	{
		...
		"<script>window.alert('Status do Coletor ( filial ) Nada Salvo ! !')
			window.location.replace('../view/form_controles.php');
		</script>";	
	}
	else
	{
...
{
	$query = "update consertoc set situacao = 'conserto', atualizacao = '$data_atualizacao1', rma = '$rma1',  nfe = '$nfe1',  defeito = '$defeito1', almox = '$data_almox1' where identificador = '$ident_post1' and filial = '$filial_usuario_logado' and situacao <> 'chegada'";
